...
struct SLoader
{
	...
	struct SHelper
	{
		...
		IC static void load_data(T& data, M& stream, const P& p)
		{
			SHelper1<T>::load_data<object_type_traits::is_base_and_derived_or_same_from_template<IPureLîadableObject, T>::value>(data, stream, p);
		}

		template <>
		IC static void load_data<true>(T& data, M& stream, const P& p)
		{
			SLoader<M, P>::load_data(*(data = xr_new<object_type_traits::remove_pointer<T>::type>( )), stream, p);
		}
	};
	...
	struct SHelper3
	{
		...
		IC static void load_data(T& data, M& stream, const P& p)
		{
			...
			{
				T::value_type temp;
				SLoader<M, P>::load_data(temp, stream, p);
				if (p(data, temp))
				{
					add(data, temp);
				}
	...
	struct SHelper4
	{
		...
		IC static void load_data(T& data, M& stream, const P& p)
		{
			SHelper<T>::load_data<object_type_traits::is_pointer<T>::value>(data, stream, p);
		}

		template <>
		IC static void load_data<true>(T& data, M& stream, const P& p)
		{
			SHelper3::load_data(data, stream, p);
		}
	};
	...
	IC static void load_data(std::queue<T1, T2>& data, M& stream, const P& p)
	{
		...
		for (u32 i = 0; i < count; ++i)
		{
			...
			SLoader<M, P>::load_data(t, stream, p);
			if (p(temp, t))
			{
				temp.push(t);
			}
	...
	IC static void load_data(T& data, M& stream, const P& p)
	{
		SHelper4<T>::load_data<object_type_traits::is_stl_container<T>::value>(data, stream, p);
	}
};
...
{
	load_data(data, stream, object_loader::detail::SEmptyPredicate( ));
}
