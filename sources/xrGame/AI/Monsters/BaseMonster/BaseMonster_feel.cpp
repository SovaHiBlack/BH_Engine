...
void CBaseMonster::feel_sound_new(CObject* who, s32 eType, CSound_UserDataPtr user_data, const fVector3& Position, f32 power)
{
	...
	if (!entity && ((eType & eST_ITEM_HIDING) == eST_ITEM_HIDING))
	{
		return;
	}
	...
	if ((eType & eST_WEAPON_SHOOTING) == eST_WEAPON_SHOOTING)
	{
		power = 1.0f;
	}

	if (((eType & eST_WEAPON_BULLET_HIT) == eST_WEAPON_BULLET_HIT) && (dist < 2.0f))
	{
...
void CBaseMonster::HitEntity(const CEntity* pEntity, f32 fDamage, f32 impulse, fVector3& dir)
{
	...
	if (EnemyMan.get_enemy( ) == pEntity)
	{
		...
		if (pEntityNC == Actor( ))
		{
			...
			//
			//////////////////////////////////////////////////////////////////////////
			CCameraEffector* ce = Actor( )->Cameras( ).GetCamEffector((ECamEffectorType)effBigMonsterHit);
			if (!ce)
			{
...
void CBaseMonster::Hit_Psy(CObject* object, f32 value)
{
	...
	HS.whoID = (ID( ));
	HS.weaponID = (ID( ));
	HS.dir = (fVector3( ).set(0.0f, 1.0f, 0.0f));
	HS.power = (value);
	HS.boneID = (BI_NONE);
