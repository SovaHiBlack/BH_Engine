...
{
	if (((stype & eST_WEAPON) != eST_WEAPON) &&
		((stype & eST_MONSTER) != eST_MONSTER) &&
		((stype & eST_WORLD) != eST_WORLD)) return NONE_DANGEROUS_SOUND;

	CHECK_SOUND_TYPE(stype, eST_WEAPON_RECHARGING,	WEAPON_RECHARGING);
	CHECK_SOUND_TYPE(stype, eST_WEAPON_SHOOTING,		WEAPON_SHOOTING);
	CHECK_SOUND_TYPE(stype, eST_ITEM_HIDING,			WEAPON_HIDING);
	CHECK_SOUND_TYPE(stype, eST_ITEM_TAKING, WEAPON_TAKING);
	CHECK_SOUND_TYPE(stype, eST_WEAPON_EMPTY_CLICKING,	WEAPON_EMPTY_CLICKING);
	CHECK_SOUND_TYPE(stype, eST_WEAPON_BULLET_HIT,	WEAPON_BULLET_RICOCHET);
	CHECK_SOUND_TYPE(stype, eST_MONSTER_DYING,		MONSTER_DYING);
	CHECK_SOUND_TYPE(stype, eST_MONSTER_INJURING,	MONSTER_INJURING);
	CHECK_SOUND_TYPE(stype, eST_MONSTER_STEP,		MONSTER_WALKING);

	CHECK_SOUND_TYPE(stype, eST_MONSTER_TALKING,		MONSTER_TALKING);
	CHECK_SOUND_TYPE(stype, eST_MONSTER_ATTACKING,	MONSTER_ATTACKING);
	CHECK_SOUND_TYPE(stype, eST_WORLD_OBJECT_BREAKING,	OBJECT_BREAKING);
	CHECK_SOUND_TYPE(stype, eST_WORLD_OBJECT_COLLIDING,	OBJECT_FALLING);
...
bool CMonsterSoundMemory::is_loud_sound(f32 val)
{
...
{
	if ((eType & eST_MONSTER_ATTACKING) != eST_MONSTER_ATTACKING)
	{
		return;
	}

	if ((eType & eST_MONSTER_INJURING) != eST_MONSTER_INJURING)
	{
		return;
	}

	if ((eType & eST_MONSTER_DYING) != eST_MONSTER_DYING)
	{
		return;
	}
