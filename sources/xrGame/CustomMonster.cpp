...
void CCustomMonster::SAnimState::Create(CKinematicsAnimated* K, pcstr base)
{
	char	buf[128];
...
void CCustomMonster::Load(pcstr section)
{
...
void CCustomMonster::reload(pcstr section)
{
...
//			VERIFY					(d2);
			...
			fVector3					l_tOldPosition = Position( );
...
void CCustomMonster::HitSignal(f32 /**perc/**/, fVector3& /**vLocalDir/**/, CObject* /**who/**/)
{ }
...
void CCustomMonster::load_killer_clsids(pcstr section)
{
	m_killer_clsids.clear( );
	pcstr							killers = pSettings->r_string(section, "killer_clsids");
	string16						temp;
...
void CCustomMonster::feel_sound_new(CObject* who, s32 type, CSound_UserDataPtr user_data, const fVector3& position, f32 power)
{
	if (getDestroy( ))
		return;
...
pcstr CCustomMonster::visual_name(CSE_Abstract* server_entity)
{
	m_already_dead = false;
...
extern void dbg_draw_frustum(f32 FOV, f32 _FAR, f32 A, fVector3& P, fVector3& D, fVector3& U);
...
void CCustomMonster::OnRender( )
{
	...
	for (s32 i = 0; i < 1; ++i)
	{
		...
		f32									radius1 = !i ? 0.2f : 0.3f;
		{
			for (u32 I = 1; I < path.size( ); ++I)
			{
				...
				P2.set(N2.position);
				P2.y += 0.1f;
				if (!fis_zero(P1.distance_to_sqr(P2), EPSILON_3))
				{
	...
	}
	{
		...
		fVector3				P1 = ai( ).level_graph( ).vertex_position(node);
		P1.y += 1.f;
	...
	if (g_Alive( ))
	{
		...
		{
			fVector3				P1 = memory( ).memory(memory( ).enemy( ).selected( )).m_object_params.m_position;
			P1.y += 1.f;
		...
		{
			fVector3				P1 = memory( ).danger( ).selected( )->position( );
			P1.y += 1.f;
			Level( ).debug_renderer( ).draw_aabb(P1, 1.f, 1.f, 1.f, D3DCOLOR_XRGB(0, 0, 0));
		}
	}
