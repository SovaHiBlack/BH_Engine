...
pcstr CScriptEntity::GetScriptControlName() const
{
...
bool CScriptEntity::CheckTypeVisibility(pcstr section_name)
{
	if (!m_monster)
		return			(false);
...
void CScriptEntity::vfUpdateSounds()
{
	...
	if (xr_strlen(l_tSoundAction.m_caBoneName) && m_current_sound && m_current_sound->_feedback())
		m_current_sound->_feedback()->set_position(GetUpdatedMatrix(l_tSoundAction.m_caBoneName,l_tSoundAction.m_tSoundPosition, fVector3().set(0.0f,0.0f,0.0f)).c);
}
...
const fMatrix4x4 CScriptEntity::GetUpdatedMatrix(shared_str caBoneName, const fVector3& tPositionOffset, const fVector3& tAngleOffset)
{
	fMatrix4x4			l_tMatrix;
...
#endif
				const fMatrix4x4& l_tMatrix = GetUpdatedMatrix(l_tSoundAction.m_caBoneName,l_tSoundAction.m_tSoundPosition,l_tSoundAction.m_tSoundAngles);
	...
	if (l_tParticleAction.m_tpParticleSystem) {
		if (true/** !l_tParticleAction.m_tpParticleSystem/**/)
			if (!l_tParticleAction.m_bStartedToPlay) {
				const fMatrix4x4& l_tMatrix = GetUpdatedMatrix(*l_tParticleAction.m_caBoneName,l_tParticleAction.m_tParticlePosition,l_tParticleAction.m_tParticleAngles);
...
pcstr CScriptEntity::GetPatrolPathName()
{
#ifdef DEBUG
	if (!GetScriptControl()) {
...
bool CScriptEntity::bfScriptAnimation()
{
	...
	if (
		...
		xr_strlen(GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay)) {
			...
			pcstr				animation_id = *GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay;
...
void CScriptEntity::sound_callback	(const CObject *object, s32 sound_type, const fVector3& position, f32 sound_power)
{
...
s32 CScriptEntity::get_enemy_strength()
{
	return (0);
}
