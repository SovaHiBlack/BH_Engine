...
{
	m_corpse_cover_evaluator->setup(10.0f, 50.0f);
	const CCoverPoint* point = ai( ).cover_manager( ).best_cover(Position( ), 30.0f, *m_corpse_cover_evaluator);
	if (!point)
	{
		return false;
	}
...
{
	m_enemy_cover_evaluator->setup(enemy_pos, 30.0f, 50.0f);
	const CCoverPoint* point = ai( ).cover_manager( ).best_cover(Position( ), 40.0f, *m_enemy_cover_evaluator);
	if (!point)
	{
		return false;
	}
