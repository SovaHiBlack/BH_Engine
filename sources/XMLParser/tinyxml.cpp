...
void TiXmlBase::PutString(const xr_string& str, xr_string* outString)
{
	...
	while (i < (s32)str.length( ))
	{
		...
		else if (c < 32)
		{
			...
			string32 buf;
			_snprintf_s(buf, sizeof(buf), "&#x%02X;", (u32)(c & 0xff));

			//*ME:	warning C4267: convert 'size_t' to 'int'
			//*ME:	Int-Cast to make compiler happy ...
			outString->append(buf, (s32)xr_strlen(buf));
			++i;
		}
		else
		{
...
pcstr TiXmlElement::Attribute(pcstr name, s32* i) const
{
...
const xr_string* TiXmlElement::Attribute(const xr_string& name, s32* i) const
{
...
pcstr TiXmlElement::Attribute(pcstr name, f64* d) const
{
	pcstr s = Attribute(name);
	if (d)
	{
...
const xr_string* TiXmlElement::Attribute(const xr_string& name, f64* d) const
{
	const xr_string* s = Attribute(name);
	if (d)
	{
...
s32 TiXmlElement::QueryIntAttribute(pcstr name, s32* ival) const
{
	const TiXmlAttribute* node = attributeSet.Find(name);
	if (!node)
	{
...
s32 TiXmlElement::QueryIntAttribute(const xr_string& name, s32* ival) const
{
	const TiXmlAttribute* node = attributeSet.Find(name);
	if (!node)
	{
...
s32 TiXmlElement::QueryDoubleAttribute(pcstr name, f64* dval) const
{
	const TiXmlAttribute* node = attributeSet.Find(name);
	if (!node)
	{
...
s32 TiXmlElement::QueryDoubleAttribute(const xr_string& name, f64* dval) const
{
	const TiXmlAttribute* node = attributeSet.Find(name);
	if (!node)
	{
...
void TiXmlElement::SetAttribute(pcstr name, s32 val)
{
	string64 buf;
...
void TiXmlElement::SetAttribute(const xr_string& name, s32 val)
{
	string64 buf;
	_snprintf_s(buf, sizeof(buf), "%d", val);
	SetAttribute(name, buf);
...
void TiXmlElement::SetDoubleAttribute(pcstr name, f64 val)
{
	string256 buf;
...
void TiXmlElement::Print(FILE* cfile, s32 depth) const
{
	s32 i;
	assert(cfile);
	for (i = 0; i < depth; i++)
...
bool TiXmlDocument::LoadFile(FILE* file, TiXmlEncoding encoding)
{
	...
	*/
	...
	pstr buf = xr_alloc<char>(length + 1);
	buf[0] = 0;
...
void TiXmlDocument::Print(FILE* cfile, s32 depth) const
{
	assert(cfile);
	for (const TiXmlNode* node = FirstChild( ); node; node = node->NextSibling( ))
	{
...
s32 TiXmlAttribute::QueryIntValue(s32* ival) const
{
...
s32 TiXmlAttribute::QueryDoubleValue(f64* dval) const
{
...
void TiXmlAttribute::SetIntValue(s32 _value)
{
	string64 buf;
...
s32 TiXmlAttribute::IntValue( ) const
{
	return atoi(value.c_str( ));
}

f64 TiXmlAttribute::DoubleValue( ) const
{
...
void TiXmlComment::Print(FILE* cfile, s32 depth) const
{
	assert(cfile);
...
void TiXmlText::Print(FILE* cfile, s32 depth) const
{
	assert(cfile);
	if (cdata)
	{
...
TiXmlDeclaration::TiXmlDeclaration(pcstr _version, pcstr _encoding, pcstr _standalone) : TiXmlNode(TiXmlNode::DECLARATION)
{
	version = _version;
	encoding = _encoding;
...
void TiXmlUnknown::Print(FILE* cfile, s32 depth) const
{
	for (s32 i = 0; i < depth; i++)
	{
		fprintf(cfile, "    ");
...
TiXmlHandle TiXmlHandle::Child(s32 count) const
{
	if (node)
	{
		s32 i;
...
TiXmlHandle TiXmlHandle::Child(pcstr value, s32 count) const
{
	if (node)
	{
		s32 i;
...
TiXmlHandle TiXmlHandle::ChildElement(s32 count) const
{
	if (node)
	{
		s32 i;
...
TiXmlHandle TiXmlHandle::ChildElement(pcstr value, s32 count) const
{
	if (node)
	{
		s32 i;
