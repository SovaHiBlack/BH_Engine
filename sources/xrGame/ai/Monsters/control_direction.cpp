...
void CControlDirection::update_frame()
{
	...
	if (!fis_zero(m_man->movement().velocity_current()) && !fis_zero(m_man->movement().velocity_target()) && m_data.linear_dependency)
		m_heading.current_speed	= m_data.heading.target_speed * m_man->movement().velocity_current() / (m_man->movement().velocity_target() + EPSILON_3);
	else 
...
bool CControlDirection::is_face_target(const fVector3& position, f32 eps_angle)
{
	f32 target_h	= fVector3().sub(position, m_object->Position()).getH();
...
f32 CControlDirection::angle_to_target(const fVector3& position)
{
	f32		angle = fVector3().sub(position, m_object->Position()).getH();
	angle		*= -1;
